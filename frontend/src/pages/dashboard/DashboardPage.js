          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <section className="lg:col-span-2 space-y-6">
              ...
              <div className="glass-card overflow-hidden shadow-2xl relative">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[#d8572a]/5 rounded-full -mr-16 -mt-16 blur-3xl opacity-50" />
                <div className="p-8 md:p-10 space-y-8 relative z-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    <div className="space-y-1.5">
                      <p className="text-[10px] font-black text-[#e5e7eb] opacity-40 uppercase tracking-[0.2em]">Institutional Email</p>
                      <p className="font-black text-white tracking-tight truncate">{user?.email || 'unassigned@dhacquisitions.com'}</p>
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-[10px] font-black text-[#e5e7eb] opacity-40 uppercase tracking-[0.2em]">Primary Contact</p>
                      <p className="font-black text-white tracking-tight">{user?.mobileNumber || '+91 - Not Set'}</p>
                    </div>
                    <div className="md:col-span-2 space-y-3">
                      <p className="text-[10px] font-black text-[#e5e7eb] opacity-40 uppercase tracking-[0.2em]">Authorization Nodes</p>
                      <div className="flex flex-wrap gap-2 pt-1">
                        {user?.roles?.map(role => (
                          <span key={role} className="px-3 py-1.5 bg-[#d8572a] text-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-lg shadow-[#d8572a]/20">
                            {role}
                          </span>
                        ))}
                      </div>
              ...
              <div className="glass-card p-8 h-full shadow-2xl relative overflow-hidden group">
                 ...
                 <div className="relative z-10 flex flex-col h-full">
                    <div className="flex items-center justify-between mb-8">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-transform group-hover:scale-110 duration-500 ${verification?.status === 'APPROVED' ? 'bg-[#d8572a] text-white shadow-[#d8572a]/20' : 'bg-orange-500 text-white shadow-orange-500/20'}`}>
                        <FiShield className="w-6 h-6" />
                      </div>
                      <div className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full border-2 ${
                        verification?.status === 'APPROVED' ? 'bg-[#E3FCF9] border-[#f7b538]/20 text-[#db7c26]' :
                    ...
                    {!verification ? (
                      <div className="space-y-4 flex-1">
                        <h3 className="font-black text-base text-white tracking-tight">KYC Pending</h3>
                        <p className="text-xs font-bold text-[#e5e7eb] opacity-60 leading-relaxed uppercase tracking-wide">Submit institutional credentials to validate your entity.</p>
                        <button 
                          onClick={() => setShowVerModal(true)}
                          className="text-[10px] font-black text-[#db7c26] hover:text-[#3a5578] transition-all uppercase tracking-[0.2em] pt-2 underline decoration-[#d8572a]/20"
                        >
                          Initiate Protocol
                        </button>
                      </div>
                      ...
                      <div className="space-y-6 flex-1">
                        <div className="space-y-3">
                          <div className="flex justify-between items-center group/data">
                             <span className="text-[9px] font-black text-[#e5e7eb] opacity-40 uppercase tracking-widest">Type</span>
                             <span className="text-xs font-black text-white truncate">{verification.docType}</span>
                          </div>
                          <div className="flex justify-between items-center group/data">
                             <span className="text-[9px] font-black text-[#e5e7eb] opacity-40 uppercase tracking-widest">Date</span>
                             <span className="text-xs font-black text-white">{new Date(verification.createdAt).toLocaleDateString()}</span>
                          </div>
                        </div>
                        ...
                        <div className="pt-4 mt-auto">
                           <div className="w-full h-1 bg-gray-100 rounded-full overflow-hidden">
                              <div className={`h-full transition-all duration-1000 ${verification?.status === 'APPROVED' ? 'w-full bg-[#d8572a]' : verification?.status === 'PENDING' ? 'w-2/3 bg-orange-500' : 'w-1/3 bg-red-500'}`} />
                           </div>
                           <p className="text-[8px] font-black text-[#e5e7eb] opacity-20 uppercase tracking-[0.3em] mt-3 text-center">Identity Node Integrity</p>
                        </div>
                      </div>
                    )}
                 </div>
